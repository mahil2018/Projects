...
RefreshAndCloseWindow = function(clearQueryParams)
{
	...
	try // without this try-catch IE throws Access Denied for dialogs called from site-admin
	{
		if (top.commonspot.util.browser.chrome || top.commonspot.util.browser.safari) //
		{
			...
			if (openFrame && (openFrame.src.indexOf('/dashboard/dialogs/siteadmin/metadata-form-addedit.html') === -1) && (openFrame.src.indexOf('/commonspot/dashboard/null') === -1) && (openFrame.src.indexOf('loader.cfm?csModule=metadata/form_control/edit&id=') === -1))
				doSpecialCase = true;
			if (doSpecialCase)
